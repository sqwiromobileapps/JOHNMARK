...
const appMappingFile = path.resolve(__dirname, './app-mapping.js');
...
async function linkApp({ srcDir, appDir, appName }) {
  try {
    if (!apps) {
      try {
        apps = {};
        apps = await readFile(appMappingFile);
    ...
    let appname = `${appName}`
      .replace('@sqwiromobilecustomapps/', '@sqwiro/customapps/')
      .replace('@sqwiromobilebusinessapps/', '@sqwiro/businessapps/')
      .replace('@sqwiromobilesuperadminapps/', '@sqwiro/superadminapps/')
      .replace('@sqwiromobileapiapps/', '@sqwiro/apiapps/')
      .replace('@sqwiromobileuserapps/', '@sqwiro/userapps/')
      .replace('@sqwiromobile/', '@sqwiro/');

    apps[
      appname
    ] = `/Users/bernardgaitho/@cloudhub/@sqwiroapps/mobileapps/${appName}/src`;
